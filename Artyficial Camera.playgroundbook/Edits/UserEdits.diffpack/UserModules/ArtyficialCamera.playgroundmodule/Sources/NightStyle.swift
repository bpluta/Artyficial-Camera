...
class NightStyleInput : MLFeatureProvider {
...
@available(macOS 10.14, iOS 12.0, tvOS 12.0, watchOS 5.0, *)
class NightStyleOutput : MLFeatureProvider {
...
class NightStyle {
    ...
    class var urlOfModelInThisBundle : URL {
        let bundle = Bundle(for: NightStyle.self)
        return bundle.url(forResource: "NightStyle", withExtension:"mlmodelc")!
    }
    ...
    /**
     ...
     - parameters:
     - input: the input to the prediction as NightStyleInput
     - throws: an NSError object that describes the problem
     - returns: the result of the prediction as NightStyleOutput
     */
    func prediction(input: NightStyleInput) throws -> NightStyleOutput {
    ...
    /**
     ...
     - returns: the result of the prediction as NightStyleOutput
     */
    func prediction(input: NightStyleInput, options: MLPredictionOptions) throws -> NightStyleOutput {
        let outFeatures = try model.prediction(from: input, options:options)
        return NightStyleOutput(features: outFeatures)
    }
    ...
    /**
     ...
     - returns: the result of the prediction as NightStyleOutput
     */
    func prediction(image: CVPixelBuffer, index: MLMultiArray) throws -> NightStyleOutput {
        let input_ = NightStyleInput(image: image, index: index)
    ...
    /**
     ...
     - parameters:
     - inputs: the inputs to the prediction as [NightStyleInput]
     - options: prediction options 
     - throws: an NSError object that describes the problem
     - returns: the result of the prediction as [NightStyleOutput]
     */
    func predictions(inputs: [NightStyleInput], options: MLPredictionOptions = MLPredictionOptions()) throws -> [NightStyleOutput] {
        let batchIn = MLArrayBatchProvider(array: inputs)
        let batchOut = try model.predictions(from: batchIn, options: options)
        var results : [NightStyleOutput] = []
        results.reserveCapacity(inputs.count)
        for i in 0..<batchOut.count {
            let outProvider = batchOut.features(at: i)
            let result =  NightStyleOutput(features: outProvider)
